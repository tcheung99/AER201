...
ROTATE  macro    PORT
	...
	org 0x00
        ...
        clrf     TRISD
        clrf     TRISE
        movlw    0x0F        ;Force analog-capable pins to digital mode
        movwf    ADCON1     
...
begin   movlw    d'1'    ;Start PORTE as 0b00000001
	...
	ROTATE   LATE    ;PORTE = 0b00000100
        ...
        ;DELAY FUNCTION
delay   ;Executes loop 65536 times, with approx. 4 instruction cycles per loop (nop = 1, decf=1, goto=2)
	;Recall that 1 instruction cycle = 4 clock cycles
	clrf	d1
	clrf	d2		 
delay1	nop
	decfsz	d1, 1	;Count from 255 to 0 (NOTE: we rely on overflow to reset the counter)
	goto	$-4	;Note: each instruction is 2 bytes long, so to go to the previous instruction, $-2 is needed	
	nop
	decfsz	d2, 1
